...
save_interval=1
Radius_board =0.3
NC_occurence=2000 #8000 = 5 times per generation. 
...
def eval_fitness(genomes,config=None,mode="DIRECT",camera=None):
  ...
  # colBoardId = p.createCollisionShape(p.GEOM_BOX, halfExtents=[Radius_board*13, Radius_board*0.4, Radius_board*0.4])
  mass_obj = 10 #nice weight for not jumped back out of valley.

  # unpickle here
  pickle_file_path = 'C:/Users/alife/niche_evo_single_species/nicheboardcase6.pkl'
  # check if file exists
  if os.path.exists(pickle_file_path):
        with open(pickle_file_path, 'rb') as file:
            nichepos = pickle.load(file)
        inheritpos = nichepos[-1]
        percent_inherit = int(len(inheritpos) * 0.03)
        for objpos in inheritpos[:percent_inherit]:
          colBoard = p.createCollisionShape(p.GEOM_BOX, halfExtents=[Radius_board*13, Radius_board*0.4, Radius_board*0.4])
          p.createMultiBody(mass_obj, colBoard, visualShapeId, objpos, baseOrientation)
  else:
  ...
  if __name__ == '__main__':
    ...
    if np.mod(len(Genomes),save_interval)==0: # if num of genomes is divisible by save_interval;
      ...
      shutil.copy(data_dir+'/nicheboard_'+str(len(Genomes))+'.pkl', 'C:/Users/alife/niche_evo_single_species/nicheboardcase4.pkl')
      ...
      if os.path.isfile(data_dir+'/nicheboard_'+str(len(Genomes)-save_interval)+'.pkl')==1:
        ...
        print("nc is pickled for this generation")
      # # Event
