...
def eval_fitness(genomes,config=None,mode="GUI",camera=None):
  ...
  # # unpickle here
  pickle_file_path = '/Users/alife/niche_evo_single_species/nicheboardei20.pkl'
  ...
  if __name__ == '__main__':
    ...
    if np.mod(len(Genomes),save_interval)==0: # if num of genomes is divisible by save_interval;
      ...
      shutil.copy(data_dir+'/nicheboard_'+str(len(Genomes))+'.pkl', '/Users/alife/niche_evo_single_species/nicheboardei20.pkl') 
...
def run(gens):
  ...
  pop = neat.Checkpointer.restore_checkpoint(os.path.dirname(__file__)+"/case5")
