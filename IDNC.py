...
save_interval=1
Radius_board =0.3
NC_occurence=20 #8000 = 5 times per generation.
...
def eval_fitness(genomes,config=None,mode="GUI",camera=None):
  ...
  ramp1=p.loadURDF('rect2.urdf',[0,34.3,-3.37], globalScaling=70, useFixedBase=True) #-3.35
  ramp2=p.loadURDF('rect2.urdf',[0,9.35,-3.37], globalScaling=70, useFixedBase=True) #ni yg dkt target
  ...
  # # unpickle here
  # pickle_file_path = '/Users/sitiaisyahjaafar/Desktop/niche_evo_single_species/nicheboard.pkl'
  ...
  #       percent_inherit = int(len(inheritpos) * 1)
  #       for objpos in inheritpos[:percent_inherit]:
  #         inheritobj=p.createMultiBody(mass_obj, colBoardId, visualShapeId, objpos, baseOrientation)
  ...
  for step in tqdm(range(Total_Step)): # progress bar (tqdm) for the loop,
    for i,(ind,g) in enumerate(genomes): # iterates over index and genomes simultaneously, ind: index, g:genome
      ...
      if type(camera)==int:
        if ind==camera:
          p.resetDebugVisualizerCamera(cameraDistance=10, cameraYaw=90, cameraPitch=-10, cameraTargetPosition=p.getBasePositionAndOrientation(g.creature.bodyId)[0])
      ...
      if len(jointlist)==0: #kalau xde joint kt creatures body,
        ...
        placePos = np.array([(creaturePosition[0]-2), (yaxes - 5), (creaturePosition[2] + 3)]) #ni dah okay sangat
        g.position=[p.getBasePositionAndOrientation(g.creature.bodyId)[0]]
        position=int(g.position[0][1])

        if np.mod(step, NC_occurence) == 0:
          if (not board_placed and activate_board) and ((40.5 <= position <= 42.5) or (15.8 <= position <= 18.5)):
            # colBoardId = p.createCollisionShape(p.GEOM_BOX, halfExtents=[Radius_board*15, Radius_board*0.25, Radius_board*0.11])
